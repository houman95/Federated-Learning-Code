...
def rand_k_sparsify_model_weights(weights, fraction):
    # Flatten and collect all weights
    flat_weights = [w.view(-1) for w in weights]
    all_weights = torch.cat(flat_weights)
    # Determine the number of elements to keep
    num_weights = len(all_weights)
    num_to_keep = int(fraction * num_weights)
    # Create a mask with random selection of elements to keep
    keep_indices = torch.randperm(num_weights)[:num_to_keep]
    mask = torch.zeros(num_weights, dtype=torch.bool)
    ...
    for w in weights:
        ...
        current_mask = mask[start_idx:start_idx + num_elements].reshape(w.shape)
        new_w = w * current_mask.float()
        ...
        for timeframe in range(num_timeframes):
            ...
            for user_id in range(num_users):
                ...
                if use_memory_matrix:
                    sparse_gradient[user_id] = rand_k_sparsificate_model_weights(gradient_diff_memory, fraction[0])
                else:
                    sparse_gradient[user_id] = rand_k_sparsificate_model_weights(gradient_diff, fraction[0])
...
save_dir = f"./RandomOracle{num_slots}slots_{memory_status}_{current_time}"
